...
export class AppleScriptFramework {
  ...
  log(level: LogLevel, message: string, data?: Record<string, any>): void {
    ...
    if (this._isConnected) {
      try {
        // SDK >=1.25 dropped the top-level `message` field; fold it into data
  ...
  private async executeScript(script: string): Promise<string> {
    ...
    try {
      ...
      const { stdout } = await execAsync(
        ...
        {
          // Default maxBuffer (1 MB) overflows on tools that return lots of
          // data (Notes/Messages). Raise it and cap runtime so a slow query
          // errors instead of hanging the agent indefinitely.
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        this.log("debug", "Resolving tool implementation", {
          categoryName,
          scriptName,
          kind: script.handler ? "handler" : "applescript"
        });

        let result: string | { content: unknown[] };
        if (script.handler) {
          // Handler-backed tool (e.g. Telegram API, screenshot): run JS directly.
          // Handlers may return a plain string or a full MCP result ({content}).
          result = await script.handler(request.params.arguments ?? {});
        } else {
          const scriptContent =
          ...
          if (scriptContent === undefined) {
            throw new McpError(
              ...
              `Tool ${toolName} has neither a script nor a handler`,
            );
          }

          result = await this.executeScript(scriptContent);
        }
        ...
        // A handler may return a full MCP result (e.g. an image); pass it through.
