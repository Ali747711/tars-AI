...
function TrackDeviceSelect({
  ...
  return (
    <Select
      ...
      // This project's Select (Base UI) reports `string | null`; the registry
      // component expects a plain deviceId callback.
      onValueChange={(value) => {
        if (typeof value === "string") onActiveDeviceChange?.(value)
      }}
    >
      <SelectTrigger
      ...
      {/* Base UI's SelectContent positions itself; no `position` prop. */}
      <SelectContent>
        {devices.map((device) => (
          <SelectItem
            key={device.deviceId}
